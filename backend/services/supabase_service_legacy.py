...
class SupabaseManager:
    ...
    * If Supabase is not configured every method returns a predictable
    ...
    def admin_delete_user_chats(cls, user_id: str) -> None:
        ...
        cls._authed_client().table("chat_history").delete().eq("user_id", user_id).execute()

    @classmethod
    def admin_delete_user_memories(cls, user_id: str) -> None:
        """Delete all memories for a specific user."""
        cls._authed_client().table("memories").delete().eq("user_id", user_id).execute()

    @classmethod
    def admin_clear_all_chats(cls) -> None:
        """Delete ALL chat history across all users."""
        cls._authed_client().table("chat_history").delete().neq("id", 0).execute()

    @classmethod
    def admin_clear_all_memories(cls) -> None:
        """Delete ALL memories across all users."""
        cls._authed_client().table("memories").delete().neq("id", 0).execute()
